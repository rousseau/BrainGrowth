...
def volume_mesh(nn, ne, tets, Ut):
  ...
  for i in prange(ne):
    ...
    x2_init = Ut[n3] - Ut[n1]
    x3_init = Ut[n4] - Ut[n1]
    A_init = np.matrix([x1_init, x2_init, x3_init])
    vol_init = np.linalg.det(A_init.transpose())/6.0
...
def volumeNodal(G, A0, Vn0, Vn, tets, Ut, ne, nn):
  for i in prange(nn):
    ...
    Vn[i] = 0.0
  for i in prange(ne):
    vol0 = np.linalg.det(np.dot(G[i], A0[i]))/6.0
