...
def load_texture(gifti_file):
    ...
    return texture.TextureND(darray=np.array(cat_darrays), metadata=nb_texture.get_meta().metadata)
...
def write_texture(tex, gifti_file):
    ...
    """
    darrays_list = []
    out_texture_data = np.copy(np.array(tex))  # or whatever you want to write
