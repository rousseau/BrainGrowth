...
def createNNLtriangle(NNLt, Ut, faces, SN, nsn, nf, hs, bw, mw):
	...
	for i in prange(nsn):   # Search cells around each surface point and build proximity list
		pt = SN[i]
		NNLt[i][:] = []
		...
		for xi in range(max(0,xa-1), min(mx-1, xa+1)+1):      # Browse head list
			for yi in range(max(0,ya-1), min(mx-1, ya+1)+1):
				for zi in range(max(0,za-1), min(mx-1, za+1)+1):
...
def contactProcess(Ut, Ft, SN, Utold, nsn, NNLt, faces, nf, bw, mw, hs, hc, kc, a, gr):
	...
	ub = vb = wb = 0.0  # Barycentric coordinates of triangles
	for i in prange(nsn):
		maxDist = max(maxDist, np.linalg.norm(Ut[SN[i]] - Utold[i]))
	if maxDist > 0.5*(hs-hc):
		...
		for i in range(nsn):
			Utold[i] = Ut[SN[i]]
	for i in prange(nsn): # Loop through surface points
		for tp in range(len(NNLt[i])):
			pt = SN[i] # A surface point index
			tri = NNLt[i][tp] # A proximity triangle index
			pc, ubt, vbt, wbt = closestPointTriangle(Ut[pt], Ut[faces[tri][0]], Ut[faces[i][1]], Ut[faces[i][2]], ub, vb, wb)  # Find the nearest point to Barycentric
			...
			if rc < hc and gr[pt] + gr[faces[tri][0]] > 0.0:  # Calculate contact force if within the contact range
				cc *= 1.0/rc
				Ntri = np.cross(Ut[faces[tri][1]] - Ut[faces[tri][0]], Ut[faces[tri][2]] - Ut[faces[tri][0]]) # Triangle normal
				Ntrr = np.linalg.norm(Ntri)
				Ntri *= 1.0/Ntrr
