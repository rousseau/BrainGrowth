...
from mechanics2 import wrapper
...
def wrap():
  return wrapper(material_tets, ref_state_tets, Ft, tan_growth_tensor, bulk_modulus, k_param, mu, tets, Vn, Vn0, n_tets, eps)
...
def tetraElasticity_np(material_tets, ref_state_tets, Ft, tan_growth_tensor, bulk_modulus, k_param, mu, tets, Vn, Vn0, n_tets, eps):
  ...
  for i in prange(n_tets):
    ...
    ll1, ll3, ll2 = np.linalg.eigvals(B[i])   #retest, some error between ll3 and ll2, longer than numa EV
